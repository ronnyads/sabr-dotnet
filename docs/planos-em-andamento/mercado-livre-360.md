...
- O webhook `orders_v2` agora sincroniza o pedido específico, sem varredura da janela inteira do seller; shipment/payment ainda usam a reconciliação ampla até haver identidade de pedido inequívoca. O teste de integração dirigido passou com a busca ampla propositalmente falhando e a suíte completa permaneceu 368/368. Publicado como Fly v165 (`943af32`), pipeline verde, API/worker iniciados e health HTTP 200.
- O admin recebeu um bloco “Saúde do acesso Billing” para acionar o probe por cliente, distinguir autorizado/pendente/risco temporário e manter explícito que ausência de Billing deixa valores estimados. Build admin de produção passou; deploy e homologação visual permanecem pendentes nesta anotação.
- Homologação autenticada concluída no Admin para o seller piloto ABERAMABLACK. O primeiro contrato do probe retornava 400 porque dependia do tenant implícito, ausente no host administrativo; a rota passou a receber `tenantSlug` explicitamente, validar tenant/cliente/conexão e foi coberta por teste HTTP. Em produção o probe respondeu 200 e o Mercado Livre retornou 429, corretamente apresentado como risco temporário — não como Billing autorizado nem valor zero. Foi adicionado cooldown persistido de cinco minutos para não insistir no endpoint após rate limit. Suíte completa 370/370, build admin aprovado, Fly v171 com API e worker iniciados e health 200. O consumidor conciliatório de Billing e o grant Mercado Pago continuam pendentes.
- Mercado Pago foi promovido a integração financeira independente no contrato do hub: card próprio no admin e no portal, listagem de clientes baseada em `MarketplaceOAuthGrant` com `AppFamily=MERCADO_PAGO`, estado de autorização/Billing e rota dedicada `/client/integrations/mercadopago`. O OAuth retorna para a tela própria; Mercado Livre permanece o canal operacional. Commits `6711dcf` (API), `bb0872c` (frontend) e ajustes de contexto Fly `b1f8b10`, `bbb2886`, `fc43ca7`, `b8de585`, `ad6d048`, `3c9ee5c`. Build API sem erros, testes direcionados 50/50 e build frontend de produção aprovados. Deploy concluído: API/worker Fly v173 e frontend Fly v109; `/health` respondeu 200 com banco e Mercado Livre saudáveis. O contexto do backend foi reduzido para 6,66 MB e o entrypoint foi normalizado para Linux; máquinas de API e worker estão iniciadas. Billing real ainda depende de grant/escopo Mercado Pago autorizado e não é considerado confirmado sem probe positivo.
