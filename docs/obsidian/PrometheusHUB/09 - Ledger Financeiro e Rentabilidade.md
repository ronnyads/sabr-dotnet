...
14. A camada de domínio aceita custo de catálogo somente com origem interna autorizada (`MASTER_PRODUCT`, `VARIANT_OVERRIDE`, `EXTERNAL_SUPPLIER` ou `PREPURCHASED_LOT`). Origens de receita do canal (`MARKETPLACE_LISTING`, `ORDER_PRICE`, `CHANNEL_PRICE`) são rejeitadas antes da persistência. Produto sem custo válido fica explicitamente `CATALOG_COST_PENDING`.
15. A data econômica do custo histórico é `PaidAt` e, na ausência, `ChannelCreatedAt`. Sem ambas, ou quando mais de uma versão de preço cobre o mesmo instante, o custo permanece pendente; não há inferência por data de importação nem escolha arbitrária entre versões sobrepostas.
