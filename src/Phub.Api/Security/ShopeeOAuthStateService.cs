...
public sealed class ShopeeOAuthStateService
{
    private const string Purpose = "sabr:shopee:oauth-state:v1";
    private static readonly TimeSpan StateTtl = TimeSpan.FromMinutes(10);
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);
    private readonly IDataProtector _protector;

    public ShopeeOAuthStateService(IDataProtectionProvider dataProtectionProvider)
    {
    ...
    {
        var payload = new ShopeeOAuthStatePayload
        {
    ...
    public bool TryReadState(string state, out ShopeeOAuthStatePayload payload)
    {
        payload = default!;
        ...
        try
        {
            ...
            var parsed = JsonSerializer.Deserialize<ShopeeOAuthStatePayload>(unprotected, JsonOptions);
            if (parsed == null)
            {
                return false;
            }
    ...
    public static string NormalizeReturnUrl(string? returnUrl)
    {
        ...
        {
            return "/client/integrations/shopee";
        }
...
public sealed class ShopeeOAuthStatePayload
{
    public string TenantId { get; set; } = string.Empty;
    public Guid ClientId { get; set; }
    public string ReturnUrl { get; set; } = "/client/integrations/shopee";
